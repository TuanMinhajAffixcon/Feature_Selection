...
# Increase the limit to, for example, 2 billion pixels (adjust as needed)
...
def main():
    ...
    with col1:
        if option=='industry':
            ...
            selected_industry = st.selectbox('Enter Industry:', industry_list)
            ...
            selected_segments = st.multiselect("Enter segments",segment_list)
...
#------------------------------------------------------------------------------------------------------------------------
    ...
    # 
        ...
        def bar_chart(df):
            ...
            bars = plt.bar(df['Column Name'], df['Percentage'],color='blue')
...
#-----------------------------------------------------------------------------------------------------------------------------------
        approach=st.radio("Select one option:", ['Plot chart only','Relationship Graph only',
        ...
        #
            ...
            if selected_segment == 'Top_3_Segments_in_each_category':
                ...
                if len(selected_segments)>0:
                    ...
                    st.pyplot(plt)
                    st.write('data count: ',custom_df_count)
                else:
                    selected_segments=[]
