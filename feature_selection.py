...
#------------------------------------------------------------------------------------------------------------------------
    ...
    # 
        ...
        #     # Remove words from 'Column Name' column
        ...
        # st.write(demographics_df)
        # Custom function to update 'dem' column
...
#-----------------------------------------------------------------------------------------------------------------------------------
        ...
        prevalence,Top_left,Top_right,Bottom_left,Bottom_right,merged_op=Plot_chart_method(df_filtered,selected_segments,result_dict)
...
#------------------------------------------------------------------------------------------------------------------------------
        ...
        with col3:
            with st.expander("View Pair wise Segments"):
                ...
                st.write('Pair Wise count',edges_df)
                ...
                # st.write('Community wise selection',communities_df)
                # csv=communities_df.to_csv(index=False).encode('utf-8')
                # st.download_button("Download Community wise selection",data=csv, file_name="Community wise selection.csv")
                ...
                st.write('Connection wise Segments',degree_df)
                csv=degree_df.to_csv(index=True).encode('utf-8')
                st.download_button("Download Connection wise Segments",data=csv, file_name="Connection wise Segments.csv")
                ...
                st.write('Influence multiple Segments groups',betweenness_df)
                csv=betweenness_df.to_csv(index=True).encode('utf-8')
                st.download_button("Download Influence multiple Segments groups",data=csv, file_name="Influence multiple Segments groups.csv")

                clossness_dict=nx.closeness_centrality(G)
                clossness_df=pd.DataFrame.from_dict(clossness_dict,orient="index",columns=['centrality']).sort_values(by='centrality', ascending=False)
                st.write('Rapid information dissemination segments',clossness_df)
                csv=clossness_df.to_csv(index=True).encode('utf-8')
                st.download_button("Download Rapid information dissemination segments",data=csv, file_name="Rapid information dissemination segments.csv")
...
#------------------------------------------------------------------------------------------------------------------------
        ...
        with col2:
            with st.expander("Custom Segments Selection"):
                Custom_Segments_Selections=st.multiselect('Select Segments: ',selected_segments)
